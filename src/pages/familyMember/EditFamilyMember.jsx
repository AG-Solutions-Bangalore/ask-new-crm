...
const EditFamilyMember = () => {
  ...
  const onUpdate = async (e) => {
    ...
    setUpdating(true);
    try {
      const res = await apiClient.put(WEB_API.updateFamilyMember(id), familymember);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-pink-50 rounded-xl">
            <FiEdit className="w-6 h-6 text-pink-700" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Update Family Member Details</h1>
            <p className="text-sm text-slate-500">Edit the details for {familymember.family_member_name}.</p>
          </div>
        </div>
        <Button onClick={() => navigate("/family-member")} variant="outline" className="flex gap-2 rounded-xl h-11 px-6 shadow-sm transition-all active:scale-95">
          <FiArrowLeft /> Back
        </Button>
      </div>

      <form onSubmit={onUpdate} className="flex justify-center animate-in fade-in slide-in-from-bottom-4 duration-500">
        <Card className="w-full max-w-3xl border-none shadow-xl bg-white overflow-hidden">
          <CardHeader className="bg-slate-50/50 border-b py-6 px-8">
            <CardTitle className="text-xl font-bold flex items-center gap-2 text-pink-700">
          ...
          <CardContent className="p-8 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-2">
                <Label>Gender</Label>
                <Select onValueChange={(v) => handleSelectChange("family_member_gender", v)} value={familymember.family_member_gender}>
              ...
              <div className="space-y-2">
                <Label htmlFor="family_member_qualification">Qualification</Label>
                <Input
            ...
            <div className="flex justify-end gap-4 pt-6 border-t border-slate-100">
                ...
                <Button 
                    type="submit" 
                    disabled={updating}
                    className="bg-pink-700 hover:bg-pink-800 text-white min-w-[200px] h-12 rounded-xl font-bold shadow-lg shadow-pink-100 transition-all active:scale-95 flex items-center justify-center gap-2"
                >
                    {updating ? (
                        <>
